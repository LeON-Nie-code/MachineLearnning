...
class DecisionTree(BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        if best_split_id != None:
            ...
            self.is_leaf = False
            self.left = DecisionTree(self.split_loss_function, self.leaf_value_estimator,
                                    self.depth + 1, self.min_sample, self.max_depth)
            self.right = DecisionTree(self.split_loss_function, self.leaf_value_estimator,
                                    self.depth + 1, self.min_sample, self.max_depth)
...
def compute_entropy(label_array):
    ...
    # TODO 2.3.1
     # 确保 label_array 是一维的
    label_array = label_array.flatten()  # 或者使用 ravel()
    
    # 将标签转为可哈希的类型（如整数或字符串）
    label_array = label_array.astype(int)  # 或根据需要转换为其他类型
